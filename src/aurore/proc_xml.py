...
def xml_to_map(xml_item: ElementTree, base=None, context=None)->dict:
    return proc_elem(xml_item, base, context)[0]


def proc_elem(elem, base, context):
    if test_pull_content(elem):
        logger.info(f"Pulling content for elem: {elem}, src: {elem.attrib['src']}")
        content, sub_state = pull_content(elem, base)
        val, _ = proc[elem.tag](content, base, context)
    else:
        content = elem
        val, sub_state = proc[elem.tag](content, base, context)
...
def pull_content(elem, base, verbose=True):
    ...
    # if verbose:
    #     logger.info(f"{elem.attrib}\n{elem.tag}\n{[i for i in elem]}\n{elem.attrib['key'] if 'key' in elem.attrib else ''}")
...
def proc_map(elem, base, context=None):
    if "base" in elem.attrib:
        base = elem.attrib["base"]
    ...
    for el in elem:
        ...
        val, sub_state = proc_elem(el, base, context)
        output.update({key : val})
        if sub_state:
            state.update({key: sub_state})
    return output, state

def proc_set(elem, base, context=None):
    if "base" in elem.attrib:
        base = elem.attrib["base"]
    output = set()
    state  = set()
...
def proc_list(elem, base, context=None):
    if "base" in elem.attrib:
        base = elem.attrib["base"]
    output = []
    state = []
...
def proc_var(elem, base, context=None):
    if context is None:
        context = {}
    ans = None
    for var in elem:
        ans, state = proc[var.tag](var,base,context)
        # ans, state = proc_elem(var, base, context)
        if "id" in var.attrib:
...
def proc_eval(elem, base, context):
    context.update({
        "os": os,
